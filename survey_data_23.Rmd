...
size_bins$longest <- as.numeric(size_bins$longest)
  
## looking at the sum longest length of corals at each site 
# with garden variable
mean_lng_plot <- size_bins %>% 
  filter(site != "167") %>% 
  filter(garden != "NA") %>% 
  filter(plot != "6") %>% 
  group_by(site, plot, genus) %>% 
  summarize(mean_lng_plot = mean(longest)) %>% 
   ungroup() %>% 
...
ggplot(mean_lng_site, aes(x = factor(site), y=mean_length, group = genus)) + 
...
adults <- size_bins %>% 
  ...
  filter(plot != "6") %>% 
  select(-width, - length, -lat, -long, - species) %>% 
  drop_na(size_bins)

## Quick look - plot to see size bins 
...
sum_size_bins <- adults %>% 
  ...
  #fct_reorder(size_bins, c(">50", "45-50","40-45", "35-40","30-35", "25-30" , "20-25", "15-20", "10-15", "5-10", "<5"))


##summarized data by SITE level
sum_siz_bins_site <- sum_size_bins %>% 
  group_by (site, genus, size_bins) %>% 
  summarize(avg_n = mean(abundance),
...
juv_longest_join <- full_join(juv_longest, num_juveniles, by =c("site", "plot")) %>% 
...
### joining density data with juvenile 
juv_density <- full_join(avg_juv, avg_adults, by = c("site", "genus")) %>% 
  filter(genus == "acr")

ggplot(data = juv_density, aes(y = density_juv , x = density_a)) + 
  geom_point(aes(color = genus)) +
  #geom_smooth(method = "lm") + 
  theme_bw()
```
